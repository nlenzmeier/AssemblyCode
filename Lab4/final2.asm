 ...
 .DATA
   ...
   INCORRECT DB  0DH,0AH, 'Sorry! Incorrect response. $'
   ...
   MAIN PROC FAR
    ...
    MOV DI, SI                    ; position first operand conversion
     ...
     CMP AL,2FH                   ; compare to determine what to perform
     JE @DIV                      ; jump to label 
     CMP AL,2BH                   ; compare to determine what to perform
     JE @ADD                      ; jump to label
     CMP AL,2DH                   ; compare to determine what to perform
...
;*********************************************************;
     ...
     @WRONG:                      ; Displays error message saying that they did not put in valid answer
     LEA DX, INCORRECT            ; prints out error message
     MOV AH, 09H                  ; proceeds on to "@Continue" to ask the message again & check to yes or no
     INT 21H

     @Continue:
     LEA DX, OP_Loop              ; prints out requested loop string
     MOV AH, 09H                  ; prints repeat message
     INT 21H
     ...
     CMP AL, 79H                  ; 79H = y
     JE @Yes
     CMP AL, 4EH                  ; compares for no
     JE @No                       ; 4EH = N
     CMP AL, 6EH                  ; 6EH = n
     JE @NO                       ; NOTE: Find out a way to NOT let other letters go through...
     JNE @WRONG                   ; THIS will not allow users to put anything in other than Y,y,N,or n
 ...
 DO_DIVIDE PROC
   ...
   ; input : AX, CX
   ; output : store binary number in CX
